...
public class GraphExperimentRepositoryTest {
    ...
    public void getExperimentByInteractionAc(){
        ...
        Assert.assertEquals("Interaction Detection Method is incorect","pull down",graphExperiment.getInteractionDetectionMethod().getShortName());
        ...
        Assert.assertEquals("Publication Title is wrong","Structural basis for Pan3 binding to Pan2 and its function in mRNA recruitment and deadenylation.",graphPublication.getTitle());
        Assert.assertEquals("Journal is wrong","EMBO J. (0261-4189)",graphPublication.getJournal());
        try {
            Assert.assertEquals("Publication Date is wrong", new SimpleDateFormat("dd/MM/yyyy").parse("01/01/2014"), graphPublication.getPublicationDate());
