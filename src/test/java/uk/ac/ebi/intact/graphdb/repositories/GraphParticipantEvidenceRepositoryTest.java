...
public class GraphParticipantEvidenceRepositoryTest {
    ...
    public void getParticipantEvidencesByInteractionAc(){
        ...
        int pageNumber=0;
        int pageSize=10;
        Page<GraphParticipantEvidence> page= graphParticipantEvidenceRepository.findByInteractionAc(interactionAc, PageRequest.of(pageNumber,pageSize));
        ...
        Assert.assertEquals("Experiment Role is wrong ","bait",graphParticipantEvidence.getExperimentalRole().getShortName());
        Assert.assertEquals("Biological Role is wrong ","unspecified role",graphParticipantEvidence.getBiologicalRole().getShortName());
        Assert.assertNotNull("Detection Methods is null ",graphParticipantEvidence.getIdentificationMethods());
        Assert.assertEquals("Detection Methods size is incorrect ",1,graphParticipantEvidence.getIdentificationMethods().size());
        Assert.assertEquals("Detection Method is wrong ","weight by comassie",graphParticipantEvidence.getIdentificationMethods().iterator().next().getShortName());
        Assert.assertNotNull("Experimental Preparation is null ",graphParticipantEvidence.getExperimentalPreparations());
        Assert.assertEquals("Experimental Preparation size is incorrect ",2,graphParticipantEvidence.getExperimentalPreparations().size());

        Assert.assertEquals("Experimental Preparation Count is wrong ",2,graphParticipantEvidence.getExperimentalPreparations().size());
        ...
        Assert.assertNotNull("Experimental Praparation : "+experimentalPreparationAc+" not present" ,experimentalPreparation);
        ...
        Assert.assertEquals("Feature is wrong","strep tag",graphParticipantEvidence.getFeatures().iterator().next().getShortName());
        Assert.assertNotNull("Interactor is null",graphParticipantEvidence.getInteractor());
        Assert.assertNotNull("Interactor Ac is not present",((GraphInteractor)graphParticipantEvidence.getInteractor()).getAc());
        ...
        Assert.assertEquals("Interactor Type is wrong","protein",graphParticipantEvidence.getInteractor().getInteractorType().getShortName());
        Assert.assertEquals("Preferred Identifier is wrong","P36102",((GraphInteractor) graphParticipantEvidence.getInteractor()).getPreferredIdentifierStr());

        Assert.assertEquals("Interactor Alias Count is wrong ",5,((GraphInteractor) graphParticipantEvidence.getInteractor()).getAliases().size());
        ...
        Assert.assertEquals("Interactor Name is wrong","pan3_yeast",graphParticipantEvidence.getInteractor().getShortName());
        Assert.assertNotNull("Interactor Species is null",graphParticipantEvidence.getInteractor().getOrganism());
        Assert.assertEquals("Interactor Species is wrong","yeast",graphParticipantEvidence.getInteractor().getOrganism().getCommonName());
