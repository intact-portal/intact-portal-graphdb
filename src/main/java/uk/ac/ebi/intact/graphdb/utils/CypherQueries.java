...
public class CypherQueries {
 ...
 //   public static final String COMM_NEIGH_OF_INTOR="MATCH (interactorA:GraphInteractor)--(binaryIE:GraphBinaryInteractionEvidence)--(interactorB:GraphInteractor) WHERE NOT (interactorA) = (interactorB) RETURN interactorA,COLLECT(binaryIE) as binaryInteractionEvidences,interactorB";
    public static final String COMM_NEIGH_OF_INTOR=  "MATCH (interactorA:GraphInteractor)--(binaryIE:GraphInteractionEvidence)--(interactorB:GraphInteractor) WITH  COLLECT(binaryIE) as interactions,interactorA,interactorB UNWIND interactions as interaction MATCH (interaction:GraphInteractionEvidence) -[experiment] -(graphExperiment:GraphExperiment)-[interactionDetectionMethod:INTERACTION_DETECTION_METHOD]-(dm_cvterm:GraphCvTerm) ,(interaction:GraphInteractionEvidence) - [interactionType:INTERACTION_TYPE] - (it_cvterm:GraphCvTerm) WHERE id(interactorA) < id(interactorB) AND  NOT (interactorA) = (interactorB) RETURN interactorA,interactorB,COLLECT(interaction) as interactions,COLLECT(experiment) as experiments,COLLECT(graphExperiment) as graphExperiments,COLLECT(interactionDetectionMethod) as interactionDetectionMethods,COLLECT(dm_cvterm) as dm_cvterms,COLLECT(interactionType) as interactionTypes,COLLECT(it_cvterm) as it_cvterms ORDER BY interactorA.ac";
    public static final String INTERACTOR_PAIR_COUNT="MATCH (interactorA:GraphInteractor)--(binaryIE:GraphBinaryInteractionEvidence)--(interactorB:GraphInteractor)  WHERE id(interactorA) < id(interactorB) AND  NOT (interactorA) = (interactorB) RETURN COUNT(DISTINCT((interactorA)--()--(interactorB)))";
}
